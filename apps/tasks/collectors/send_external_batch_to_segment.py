...
def send_external_batch_to_segment(execution_id: int | None = None, event_id: int | None = None, **kwargs) -> dict:
    ...
    Args:
        execution_id: TaskExecution ID (provided by execute_db_task, ignored here)
    ...
    try:
        event = ExternalEvent.objects.get(pk=event_id)
    ...
    result = send_to_segment(
        ...
        event_name=event.segment_event_name,
    ...
    if result.get("status") == "success":
        ...
        logger.info("Sent ExternalEvent %s (%s) to Segment", event_id, event.segment_event_name)
    else:
        event.retry_count += 1
        event.status = "failed"
